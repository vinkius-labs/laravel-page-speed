...
| `RemoveQuotes` _(optional)_                   | Aggressive  | Removes quotes from simple attributes              | Global toggle; off by default   |
| `TrimUrls` _(optional)_                       | Aggressive  | Rewrites absolute URLs to relative paths           | Global toggle; off by default   |
...
1. ✅ Verify middleware is enabled in `app/Http/Kernel.php` (Laravel 10) or `bootstrap/app.php` (Laravel 11/12/13)
